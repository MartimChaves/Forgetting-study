...
def parse_args():
    ...
    parser.add_argument('--first_stage_num_classes', type=int, default=100, help='number of classes for the first stage of training')
...
def main(args):
    ...
    if args.save_best_AUC_model:
        scd_stg_data['highest_auc_loss'] = highest_auc_loss # last array will correspond to the loss when the auc was highest 
        
    if args.track_CE:
        scd_stg_data['highest_auc_ce'] = highest_auc_ce # equivalent to previous but wrt CE
        scd_stg_data['second_stage_CE_track'] = second_stage_CE_track # track CE throughout the second stage
...
def graph_hist(measure,noisy_indexes,noisy_labels,plt_name,ratio=0.2,measure_info=["Loss"]):
    ...
    try:
        ...
        if noisy_in_high_loss[0]:
            percent_noisy_in_HiLoss = 1
        else:
            percent_noisy_in_HiLoss = 0
    ...
    try:
        ...
        if noisy_in_high_loss[0]:
            percent_clean_in_LoLoss = 0
        else:
            percent_clean_in_LoLoss = 1
