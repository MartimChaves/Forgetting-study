...
def track_training_loss_plus(args, model, device, train_loader, epoch, fixed_last_layer=False,second_stage_CE_track='',calcBmm=False):
    model.eval()
    ...
    with torch.no_grad():
        ...
        if fixed_last_layer and args.track_CE: # testset is not ready
            ...
            if type(second_stage_CE_track) == list:
                second_stage_CE_track.append(neighbour_CE)
...
def train_CrossEntropy(args, model, device, train_loader, optimizer, epoch, \
                        ...
                        loss_per_epoch_train='',neighbour_CE_per_epoch_train='',\
                        fixed_last_layer=False,noisy_labels='',highest_auc_loss='',\
                        second_stage_CE_track='',highest_auc_ce=''):
    ...
    set_auc_values = False # best_auc_set
    ...
    for batch_idx, (images, labels, soft_labels, index, _, rot_img, rot_labels) in enumerate(train_loader):
        ...
        if fixed_last_layer:
            ...
            if epoch==1 and counter == 1 and args.freeze_earlySecondStage: 
                freeze_model(model)
        ...
        if track_old_data_batchWise:
            ...
            if args.save_best_AUC_model and not highest_auc_loss and not set_auc_values:
                set_auc_values = True
                best_auc = -1
            elif args.save_best_AUC_model and highest_auc_loss and not set_auc_values:
                fpr, tpr, _ = roc_curve(noisy_labels, highest_auc_loss[-1])                        
            ...
            if args.freeze_earlySecondStage and args.freeze_epochWise:
                ...
                if args.track_CE:
                    epoch_losses_train  = track_training_loss_plus(args, model, device, train_loader_cifar, epoch,fixed_last_layer=fixed_last_layer,second_stage_CE_track=second_stage_CE_track)
                else:
                    epoch_losses_train  = track_training_loss_plus(args, model, device, train_loader_cifar, epoch,fixed_last_layer=fixed_last_layer)
                ...
                if args.save_best_AUC_model:
                    ...
                    if current_auc > best_auc and epoch_losses_train.mean(axis=0) > 0.2:
                        ...
                        highest_auc_loss.append(epoch_losses_train)
                        if args.track_CE:
                            highest_auc_ce.append(second_stage_CE_track[-1])
                
                if counter - 1 > args.step_number or epoch_losses_train.mean(axis=0) > args.second_stg_max_median_loss:
                    if args.save_best_AUC_model: 
                        if not highest_auc_loss:
                            highest_auc_loss.append(epoch_losses_train)
                        if not highest_auc_ce and args.track_CE and type(highest_auc_ce)==list:    
                            highest_auc_ce.append(second_stage_CE_track[-1])
                    break
